...
def binarray(img,ndown, axis=None):
    ...
    # 2020-03-25 IJMC: Cast 'nel' as int.
    ...
    if img.ndim==2:
        ...
        nrows = int(ndownx * np.floor(nrows0/ndownx))
        ncols = int(ndowny * np.floor(ncols0/ndowny))

        nel = int(nrows*ncols/ndownx/ndowny)
        img2 = img[0:nrows,0:ncols].reshape(nel,ndownx,ndowny).sum(2)
        img_ds = img2.reshape(int(nrows/ndownx),ndownx, int(ncols/ndowny)).sum(1)

    elif img.ndim==1:
        niter = int(np.floor(img.size / ndown))
...
def pad(inp, npix_rows, npix_cols=None):
    ...
    # 2019-11-22 10:16 IJMC: Cast indices explicitly as int for Py3
    ...
    ixc = int(floor(ncols/2 + 1));
    iyc = int(floor(nrows/2 + 1));

    oxc = int(floor(npix_cols/2 + 1));
    oyc = int(floor(npix_rows/2 + 1));

    dx = npix_cols-ncols;
    dy = npix_rows-nrows;

    if dx<=0:
        ix1 = ixc - int(floor(npix_cols/2));
        ix2 = ix1 + npix_cols - 1;
        ox1 = 1;
        ox2 = npix_cols;
    else:
        ix1 = 1;
        ix2 = ncols;
        ox1 = oxc - int(floor(ncols/2));
        ox2 = ox1 + ncols - 1;

   
    if dy<=0:
        iy1 = iyc - int(floor(npix_rows/2));
        iy2 = iy1 + npix_rows - 1;
        oy1 = 1;
        oy2 = npix_rows;
    else:
...
def stdres(data, bins=None, oversamp=None, dataindex=None):
    ...
    :EXAMPLE:
      ::
        ...
        npts = 1e4
        t = np.arange(npts)
        data = np.random.normal(size=int(npts))
...
def prayerbead(*arg, **kw):
    ...
    #pdb.set_trace()
    if pool is None:
        allfits[1:] = np.array(map(pb_helperfunction, [[ii, bestmodel, bestparams, residuals, weights, modelfunction, helperargs, maxiter, maxfun, xtol, ftol, ndata, kw, verbose] for ii in range(1, ndata)])) 
    else:
        allfits[1:] = np.array(pool.map(pb_helperfunction, [[ii, bestmodel, bestparams, residuals, weights, modelfunction, helperargs, maxiter, maxfun, xtol, ftol, ndata, kw, verbose] for ii in range(1, ndata)])) 
...
def fmin_helper2(all_args):
    ...
    :EXAMPLE:
      ::
        ...
        fits = pool.map(an.fmin_helper2, [[pc.errfunc, pos0[jj], mcargs, test_kws] for jj in range(0, nwalkers, int(nwalkers/12.))])

        # The above line is equivalent to, but roughly ~(nthreads)
        # times faster, than the standard way to do it:
        fits2 = [an.fmin(pc.errfunc, pos0[jj], mcargs, **test_kw) for jj in range(0, nwalkers, int(nwalkers/10.))]
...
def returnSections(time, dtmax=0.1):
    ...
    ret = [[inds[ii]+1, inds[ii+1]] for ii in range(inds.size-1)]
    ret[-1][-1] -= 1
    return ret
...
def confmap(map, frac, **kw):
    ...
    if 'ordinate' in kw and kw['ordinate'] is not None:
...
def tsm(mp, rp, rs, teff, jmag, ars=None, a=None, p=None, ms=None, nsamp=10000):
    """Calculate TSM of Kempton et al. (2018).

    Mp and Rp should be in Earth units.

    Optional inputs must include either:     ars OR a OR ms & p (in days)

    Inputs can be single value, or 2-tuples with (value, uncertainty).

    Only VALID values will be returned.
    """
    # 2020-08-27 10:31 IJMC: Created

    nsamp = int(nsamp)
    
    # Make arrays:
    mp = np.array([mp]).ravel()
    rp = np.array([rp]).ravel()
    rs = np.array([rs]).ravel()
    teff = np.array([teff]).ravel()
    jmag = np.array([jmag]).ravel()

    # make length 2:
    if mp.size<2: mp = np.array([mp[0], 0.])
    if rp.size<2: rp = np.array([rp[0], 0.])
    if rs.size<2: rs = np.array([rs[0], 0.])
    if teff.size<2: teff = np.array([teff[0], 0.])
    if jmag.size<2: jmag = np.array([jmag[0], 0.])

    # Monte Carlo:
    mp = np.random.normal(mp[0], mp[1], nsamp)
    rp = np.random.normal(rp[0], rp[1], nsamp)
    rs = np.random.normal(rs[0], rs[1], nsamp)
    teff = np.random.normal(teff[0], teff[1], nsamp)
    jmag = np.random.normal(jmag[0], jmag[1], nsamp)
    
    if ars is None:
        if a is not None:
            a = np.array([a]).ravel()
            if a.size<2: a = np.array([a[0], 0.])
            a = np.random.normal(a[0], a[1], nsamp)
            
            ars = a*AU/(rs*rsun)
            
        elif (p is not None) and (ms is not None):
            p = np.array([p]).ravel()
            ms = np.array([ms]).ravel()
            if p.size<2: p = np.array([p[0], 0.])
            if ms.size<2: ms = np.array([ms[0], 0.])
            p = np.random.normal(p[0], p[1], nsamp)
            ms = np.random.normal(ms[0], ms[1], nsamp)
            
            a = ((p/365.24)**2 * ms)**0.333333
            #pdb.set_trace()
            ars = a*AU/(rs*rsun)
    else:
        ars = np.array([ars]).ravel()
        if ars.size<2: ars = np.array([ars[0], 0.])
        ars = np.random.normal(ars[0], ars[1], nsamp)
        
            
    scale = np.zeros(nsamp, dtype=float)
    scale[rp<1.5] = 0.19
    scale[np.logical_and(rp>=1.5, rp<2.75)] = 1.26
    scale[np.logical_and(rp>=2.75, rp<4.0)] = 1.28
    scale[rp>=4] = 1.15
    
    teq = teff/np.sqrt(ars) * 0.25**0.25 
    tsms = scale * rp**3*teq/mp/rs**2 * 10**(-jmag/5.)  

    valid = np.logical_and(mp>0, rp>0)
    valid = np.logical_and(valid, rs>0)
    valid = np.logical_and(valid, teff>0)
    valid = np.logical_and(valid, ars>1)

    if tsms[valid].std()<1e-10:
        tsms = np.array([tsms[valid].mean()])
        valid = np.array([True])

    # for val in (mp,rp,rs,teff,jmag,ars, a):
